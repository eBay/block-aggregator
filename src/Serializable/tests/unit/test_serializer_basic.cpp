...
class SerializerForProtobufRelatedTest : public ::testing::Test {
  protected:
...
ContextWrapper* SerializerForProtobufRelatedTest::shared_context = nullptr;
 ...
 *
  create table simple_event_7 (
     Count UInt64,
     Host String,
     Colo String)

  ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/simple_event_7', '{replica}') ORDER BY(Host, Count) SETTINGS
 index_granularity=8192;

  insert into simple_event_7 (`Count`, `Host`, `Colo`) VALUES (2000, 'graphdb-1', 'LVS');
 */
TEST_F(SerializerForProtobufRelatedTest, testSerializationOnNumberAndStringSingleRow) {
    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;

    DB::ContextMutablePtr context = SerializerForProtobufRelatedTest::shared_context->getContext();
    boost::asio::io_context& ioc = SerializerForProtobufRelatedTest::shared_context->getIOContext();
    ...
    std::string table = "simple_event_7";
    std::string sql = "insert into simple_event_7 values(?, ?, ?)";
...
/**
 *  create table simple_event_34 (
     Counter UInt64,
     Host String,
     Colo String,
     FlightDate Date)

    ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/simple_event_34', '{replica}') ORDER BY(Host, Counter)
 SETTINGS index_granularity=8192;

   insert into simple_event_34 (`Counter`, `Host`, `Colo`, `FlightDate`) VALUES (2000, 'graphdb-1', 'LVS',
 '2020-06-30');
 */
TEST_F(SerializerForProtobufRelatedTest, testSerializationOnNumberAndStringMultipleRows) {
    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;
    ...
    std::string table = "simple_event_34";
    std::string sql = "insert into simple_event_34 values(?, ?, ?, ?)"; // with date at the end.
    std::string shard = "nudata.monstor.cdc.dev.marketing.1";
