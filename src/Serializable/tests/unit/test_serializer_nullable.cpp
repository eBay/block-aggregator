...
class SerializerForProtobufWithNullableRelatedTest : public ::testing::Test {
  protected:
...
ContextWrapper* SerializerForProtobufWithNullableRelatedTest::shared_context = nullptr;
 ...
 */
TEST_F(SerializerForProtobufWithNullableRelatedTest, testSerializerConstructor) {
    bool failed = false;
    try {
        ...
        nuclm::SerializableDataTypePtr resulted_ptr =
            std::make_shared<nuclm::SerializableDataTypeNullable>(serializableDataTypePtr, trueDataTypePtr);
 ...
 *
 * create table simple_event_22 (
     Counter Nullable(UInt64),
     Host String,
     Colo String)

   ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/simple_event_22', '{replica}') ORDER BY(Host) SETTINGS
 index_granularity=8192;

   insert into simple_event_22 (`Counter`, `Host`, `Colo`) VALUES (2000, 'graphdb-1', 'LVS');
 */
TEST_F(SerializerForProtobufWithNullableRelatedTest, testSerializationOnNullableNumberAndNotNullStringSingleRow) {

    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;

    DB::ContextMutablePtr context = SerializerForProtobufWithNullableRelatedTest::shared_context->getContext();
    boost::asio::io_context& ioc = SerializerForProtobufWithNullableRelatedTest::shared_context->getIOContext();
    ...
    std::string table = "simple_event_22";
    std::string sql = "insert into simple_event_22 values(?, ?, ?)";
    ...
    nucolumnar::aggregator::v1::DataBindingList* dataBindingList = sqlWithBatchBindings->add_batch_bindings();
    {
        ...
        val1->set_null_value(nullValueP);
    ...
    table_definition.addColumnDescription(nuclm::TableColumnDescription("Counter", "Nullable(UInt64)"));
...
/**
 * create table simple_event_24 (
     Counter UInt64,
     Host Nullable(String),
     Colo Nullable(String))

   ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/simple_event_24', '{replica}') ORDER BY(Counter) SETTINGS
 index_granularity=8192;

  insert into simple_event_24 (`Counter`, `Host`, `Colo`) VALUES (2000, 'graphdb-1', 'LVS');
 */
TEST_F(SerializerForProtobufWithNullableRelatedTest, testSerializationOnNumberAndNotNullStringSingleRow) {
    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;
    ...
    std::string table = "simple_event_24";
    std::string sql = "insert into simple_event_24 values(?, ?, ?)";
    std::string shard = "nudata.monstor.cdc.dev.marketing.1";
    ...
    table_definition.addColumnDescription(nuclm::TableColumnDescription("Host", "Nullable (String)"));
    table_definition.addColumnDescription(nuclm::TableColumnDescription("Colo", "Nullable (String)"));
 ...
 *
 */
TEST_F(SerializerForProtobufWithNullableRelatedTest, testSerializationOnNumberAndWithNullStringSingleRow) {
    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;
    ...
    nucolumnar::aggregator::v1::DataBindingList* dataBindingList = sqlWithBatchBindings->add_batch_bindings();
    {
        ...
        // null value;
        nucolumnar::datatypes::v1::NullValueP nullValueP = nucolumnar::datatypes::v1::NullValueP::NULL_VALUE;
        val2->set_null_value(nullValueP);

        // value 3
        nucolumnar::datatypes::v1::ValueP* val3 = bindingList.add_values();
        val3->set_null_value(nullValueP);
...
/**
 * row 1:  null on first string, not null on second string
 * row 2:  null on first string, null on second string
 * row 3:  not null on first string, not null on second string.
 *
  create table simple_event_26 (
     Counter UInt64,
     Host Nullable (String),
     Colo Nullable (String),
     FlightDate Date)

  ENGINE = ReplicatedMergeTree('/clickhouse/tables/{shard}/simple_event_26', '{replica}') ORDER BY(Counter) SETTINGS
 index_granularity=8192;

  insert into simple_event_26 (`Counter`, `Host`, `Colo`, `FlightDate`) VALUES (2000, 'graphdb-1', 'LVS', '2020-06-30');

 */
TEST_F(SerializerForProtobufWithNullableRelatedTest, testSerializationOnNumberAndNullStringMultipleRows) {
    std::string path = getConfigFilePath("example_aggregator_config.json");
    LOG(INFO) << " JSON configuration file path is: " << path;
    ...
    std::string table = "simple_event_26";
    std::string sql = "insert into simple_event_26 values(?, ?, ?, ?)"; // with date at the end.
    std::string shard = "nudata.monstor.cdc.dev.marketing.1";
...

    {
        ...
        // value 2, not null.
        nucolumnar::datatypes::v1::ValueP* val2 = bindingList.add_values();
        val2->set_string_value("abc99999");
        // value 3, not null.
        nucolumnar::datatypes::v1::ValueP* val3 = bindingList.add_values();
        val3->set_string_value("xyz99999");
